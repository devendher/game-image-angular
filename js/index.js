...
var app = angular.module('cards', ['ngAnimate','ui.bootstrap']);

app.controller("gameController", function($scope, $timeout,$modal,$log) {
	$scope.showModal = false;
	$scope.startRow = 2;
	$scope.startCols = 2;
	$scope.deck = createDeck($scope.startRow,$scope.startCols);
	...
	$scope.confirmation= function(event){
		$scope.showDialogbox = false;
		if(event=== 'yes'){
			$scope.deck = createDeck($scope.startRow+2,$scope.startCols+2);
		} else {
